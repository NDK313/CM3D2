...
{
    using DataPair = Tuple<Config.DivisionCode, string>;
    using ConfigDataPair = KeyValuePair<string, Tuple<Config.DivisionCode, string>>;
    using ConfigData = SortedDictionary<string, Tuple<Config.DivisionCode, string>>;

    /// <summary>
    ///     접근 가능한 설정을 담고 있는 추상 클래스입니다.
    /// </summary>
    public abstract class AccessibleConfig : Config
    {
        ...
        /// </summary>
        protected AccessibleConfig() : base()
        {
        ...
        protected AccessibleConfig(IComparer<string> configDataComparer) : base()
        {
        ...
        public override Tuple<DivisionCode, string> AccessConfig(string configName, string defaultValue)
        {
            ...
            {
                if (dataPair.t1 == DivisionCode.Disable)
